(() => {
  ...
  function drawHeader(){
    const L=lv(), ev=evaluate(); rr(14,14,W-28,105,18); ctx.fillStyle="#fff8e9"; ctx.fill(); ctx.strokeStyle=C.ink; ctx.lineWidth=3; ctx.stroke();
    text(`${L.name}   ${state.level+1}/10`,28,38,15,950); text(`乘客：${L.passenger}`,28,64,12,850,"left","#5d4025"); text(`${L.wish.emoji} 目标：${L.wish.item}  ¥${L.target}`,28,91,12,900,"left","#5d4025");
    rr(278,27,78,32,12); ctx.fillStyle="#fff0b8"; ctx.fill(); ctx.strokeStyle=C.ink; ctx.lineWidth=2; ctx.stroke(); text(`¥${state.status==="drive"?Math.round(state.car.money):ev.money}`,317,43,20,950,"center"); text(`疑 ${Math.min(999,ev.suspicion)}/${L.patience}`,317,75,12,900,"center",ev.suspicion>=L.patience?C.red:ev.suspicion>70?"#c36b00":C.green); text(`最短 ${(ev.shortest/100).toFixed(1)}km`,317,96,10,800,"center","#79624c");
  }

  function drawBottom(){ const ev=evaluate(), L=lv(); rr(14,615,W-28,52,16); ctx.fillStyle="#fff8e9"; ctx.fill(); ctx.strokeStyle=C.ink; ctx.lineWidth=3; ctx.stroke(); let msg="沿道路拖动，经过路口会自动生成导航路线。"; if(state.screen==="start") msg="点击画面开始接单。"; else if(state.screen==="hint") msg="点击提示关闭，然后从黄色起点沿道路画到绿色终点。"; else if(state.status==="preview") msg=`预计 ¥${ev.money}/${L.target}｜绕路 ${ev.ratio.toFixed(2)}x｜怀疑 ${ev.suspicion}/${L.patience}`; else if(state.status==="drive") msg=`导航中：已赚 ¥${Math.round(state.car.money)}，目标是 ${L.wish.item}`; else if(state.status==="result") msg=ev.pass?`${"★".repeat(ev.stars)}${"☆".repeat(3-ev.stars)} 通关！买到 ${L.wish.emoji}｜¥${ev.money}`:ev.failed?`失败：乘客投诉了，怀疑值爆表。`:`未通关：只赚 ¥${ev.money}，还没够买 ${L.wish.emoji}`; text(msg.slice(0,42),28,641,13,900,"left"); }

  function drawStart(){ ctx.clearRect(0,0,W,H); const g=ctx.createLinearGradient(0,0,0,H); g.addColorStop(0,"#fff7df"); g.addColorStop(1,"#ffc978"); ctx.fillStyle=g; ctx.fillRect(0,0,W,H); ctx.save(); ctx.globalAlpha=.42; ctx.strokeStyle="#fff"; ctx.lineWidth=22; ctx.lineCap="round"; for(let i=0;i<6;i++){ctx.beginPath();ctx.moveTo(-40,140+i*82);ctx.bezierCurveTo(95,90+i*80,250,230+i*45,430,150+i*86);ctx.stroke()} ctx.strokeStyle="#ffd461"; ctx.lineWidth=11; for(let i=0;i<5;i++){ctx.beginPath();ctx.moveTo(36+i*72,80);ctx.lineTo(60+i*55,620);ctx.stroke()} ctx.restore(); rr(32,70,W-64,104,28); ctx.fillStyle="rgba(255,250,240,.92)"; ctx.fill(); ctx.strokeStyle=C.ink; ctx.lineWidth=3; ctx.stroke(); text("EVIL TAXI",W/2,105,32,950,"center"); text("绕路司机",W/2,143,22,950,"center","#ff7a1d"); drawCar(W/2,283,0,1.75); rr(52,370,W-104,118,24); ctx.fillStyle="rgba(255,250,240,.92)"; ctx.fill(); ctx.stroke(); text("闯关目标",W/2,399,18,950,"center"); text("赚够司机想买的东西",W/2,429,13,850,"center","#5e3b1e"); text("但别让乘客怀疑爆表",W/2,453,13,850,"center","#5e3b1e"); rr(70,522,W-140,54,24); ctx.fillStyle="#ff8a1f"; ctx.fill(); ctx.stroke(); text("点击开始接单",W/2,549,18,950,"center","#fffaf0"); text("路线图论版 / 10关难度递进",W/2,622,12,900,"center","#6b4b2e"); }
  ...
  function draw(){ if(state.screen==="start"){drawStart();return;} ctx.fillStyle="#f5f3ec"; ctx.fillRect(0,0,W,H); drawHeader(); drawMap(); if(state.status==="drive"){ const cp=carPoint(); drawCar(cp.x,cp.y,cp.ang,.9); } else if(state.route.length<2){ const s=node(lv().start); drawCar(s.x,s.y-42,-Math.PI/2,.7); } drawBottom(); drawHint(); }
  ...
  function refresh(){ const ev=evaluate(), L=lv(); resetBtn.disabled = state.screen==="start" || state.status==="drive"; driveBtn.disabled = !(state.screen==="game" && state.status==="preview" && ev.ended); nextBtn.disabled = !(state.status==="result" && ev.pass); if(state.screen==="start") statusEl.textContent="点击画面开始接单。"; else if(state.screen==="hint") statusEl.textContent=`本关要赚 ¥${L.target} 买${L.wish.item}。点击画面关闭提示。`; else if(state.status==="draw") statusEl.textContent="从黄色起点出发，沿道路拖动到绿色终点。建筑、河面、非道路区域不能走。"; else if(state.status==="preview") statusEl.textContent=`路线完成。预计 ¥${ev.money}，绕路 ${ev.ratio.toFixed(2)}x，怀疑 ${ev.suspicion}/${L.patience}。${ev.risk.length?"危险路段："+ev.risk.join("、")+"。":""}${ev.excuse.length?"借口："+ev.excuse.join("、")+"。":""}`; else if(state.status==="drive") statusEl.textContent="小车正在按你的路线跑，收入实时上涨。"; else if(state.status==="result") statusEl.textContent=ev.pass?`通关！${ev.stars}星，赚到 ¥${ev.money}，可以买${L.wish.item}。`:ev.failed?`失败：怀疑值 ${ev.suspicion}/${L.patience}，乘客投诉了。`:`未通关：赚到 ¥${ev.money}，还没够 ¥${L.target}，需要更黑心一点。`; }

  function startDrive(){ const ev=evaluate(); if(!ev.ended) return; state.status="drive"; state.result=null; state.car={i:0,t:0,money:lv().base,last:performance.now()}; refresh(); requestAnimationFrame(anim); }
  function anim(now){ if(state.status!=="drive") return; const pts=routePoints(); const dt=Math.min(34,now-(state.car.last||now)); state.car.last=now; let speed=2.75*dt/16.67; while(speed>0 && state.car.i<pts.length-1){ const a=pts[state.car.i], b=pts[state.car.i+1], seg=dist(a,b), remain=(1-state.car.t)*seg; if(speed<remain){ state.car.t += speed/seg; speed=0; } else { speed-=remain; state.car.i++; state.car.t=0; } }
    const doneLen = routeLen(state.route.slice(0,Math.min(state.car.i+1,state.route.length))) + (state.car.i<pts.length-1 ? dist(pts[state.car.i],pts[state.car.i+1])*state.car.t : 0); state.car.money=moneyFor(doneLen); draw();
  ...
  function reset(showHint=false){ state.route=[]; state.drawing=false; state.status="draw"; state.result=null; state.car={i:0,t:0,money:lv().base}; if(showHint) state.screen="hint"; refresh(); draw(); }
